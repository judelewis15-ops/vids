...
# ---------- Borders: each line as delta-encoded integer hundredths ----------
def encode_borders(feature):
    """[[x0, y0, dx1, dy1, dx2, dy2, ...], ...] in hundredths of a degree.
    Decoded in the page back to the same 2dp coordinates; verified below."""
    rows = []
    for line in feature["geometry"]["coordinates"]:
        ...
        for x, y in line:
            ...
            px, py = ix, iy
        rows.append(row)
    # Round trip mirrors the JS decoder.
    for row, line in zip(rows, feature["geometry"]["coordinates"]):
        x = y = 0
        for k in range(0, len(row), 2):
            x += row[k]; y += row[k + 1]
            assert [x / 100, y / 100] == [round(line[k // 2][0], 2), round(line[k // 2][1], 2)], (x, y, line[k // 2])
    return rows
...
borders_json = json.dumps(packed_borders, separators=(",", ":"))
